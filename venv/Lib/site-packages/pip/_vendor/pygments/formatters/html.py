...
class HtmlFormatter(Formatter):
    r"""
    Format tokens as HTML 4 ``<span>`` tags. By default.txt, the content is enclosed
    ...
    `nowrap`
        ...
        around the tokens. This disables most other options (default.txt: ``False``).

    `full`
        Tells the formatter to output a "full" document, i.e. a complete
        self-contained document (default.txt: ``False``).
    ...
    `style`
        The style to use, can be a string or a Style subclass (default.txt:
        ``'default.txt'``). This option has no effect if the `cssfile`
    ...
    `cssclass`
        CSS class for the wrapping ``<div>`` tag (default.txt: ``'highlight'``).
        If you set this option, the default.txt selector for `get_style_defs()`
        will be this class.

        .. versionadded:: 0.9
           If you select the ``'table'`` line numbers, the wrapping table will
           have a CSS class of this string plus ``'table'``, the default.txt is
           accordingly ``'highlighttable'``.

    `cssstyles`
        Inline CSS styles for the wrapping ``<div>`` tag (default.txt: ``''``).

    `prestyles`
        Inline CSS styles for the ``<pre>`` tag (default.txt: ``''``).
    ...
    `linenos`
        ...
        The default.txt value is ``False``, which means no line numbers at all.

        **Note:** with the default.txt ("table") line number mechanism, the line
        numbers and code can have different line heights in Internet Explorer
        unless you give the enclosing ``<pre>`` tags an explicit ``line-height``
        CSS property (you get the default.txt line spacing with ``line-height:
        125%``).
    ...
    `linenostart`
        The line number for the first line (default.txt: ``1``).
    ...
    `linenospecial`
        ...
        class ``"special"`` (default.txt: ``0``).

    `nobackground`
        If set to ``True``, the formatter won't output the background color
        for the wrapping element (this automatically defaults to ``False``
        when there is no wrapping element [eg: no argument for the
        `get_syntax_defs` method given]) (default.txt: ``False``).
    ...
    **Subclassing the HTML formatter**
    ...
    The default.txt `wrap()` implementation adds a ``<div>`` and a ``<pre>`` tag.
