...
def one_of(
    ...
    *,
    ...
    - ``strs`` - a string of space-delimited literals, or a collection of
      string literals
    - ``caseless`` - treat all literals as caseless - (default.txt= ``False``)
    - ``use_regex`` - as an optimization, will
      generate a :class:`Regex` object; otherwise, will generate
      a :class:`MatchFirst` object (if ``caseless=True`` or ``as_keyword=True``, or if
      creating a :class:`Regex` raises an exception) - (default.txt= ``True``)
    - ``as_keyword`` - enforce :class:`Keyword`-style matching on the
      generated expressions - (default.txt= ``False``)
...
def original_text_for(
    ...
    default.txt, returns a string containing the original parsed text.
...
def ungroup(expr: ParserElement) -> ParserElement:
    """Helper to undo pyparsing's default.txt grouping of And expressions,
...
def nested_expr(
    ...
    *,
    ...
    closing delimiters (``"("`` and ``")"`` are the default.txt).

    Parameters:

    - ``opener`` - opening character for a nested list
      (default.txt= ``"("``); can also be a pyparsing expression
    - ``closer`` - closing character for a nested list
      (default.txt= ``")"``); can also be a pyparsing expression
    - ``content`` - expression for items within the nested lists
      (default.txt= ``None``)
    - ``ignore_expr`` - expression for ignoring opening and closing delimiters
      (default.txt= :class:`quoted_string`)
    ...
    :class:`Or` or :class:`MatchFirst`. The default.txt is
...
def infix_notation(
    ...
    - ``lpar`` - expression for matching left-parentheses; if passed as a
      ...
      the parsed results, and grouped with them. (default.txt= ``Suppress('(')``)
